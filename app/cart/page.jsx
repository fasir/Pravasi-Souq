  ...
  return (
    <main className="flex-grow bg-[#FCFBFA] py-8">
      ...
      <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Breadcrumbs */}
        <nav className="flex items-center gap-1 text-[11px] font-bold text-brand-charcoal/40 uppercase tracking-widest mb-8">
          <Link href="/" className="hover:text-brand-primary transition-colors">Home</Link>
          <ChevronRight className="w-3.5 h-3.5" />
          <Link href="/products" className="hover:text-brand-primary transition-colors">Shop</Link>
          <ChevronRight className="w-3.5 h-3.5" />
          <span className="text-brand-charcoal/70">Cart</span>
        </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-14">
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-white border border-brand-charcoal/5 rounded-[24px] p-6 sm:p-8 shadow-xs">
              
              {/* Header */}
              <div className="border-b border-brand-charcoal/5 pb-4 mb-6">
                <h1 className="text-2xl font-black text-brand-charcoal flex items-baseline gap-2">
                  <span>Cart</span>
              ...
              {cartItems.length === 0 ? (
                <div className="text-center py-12 space-y-4">
                  <div className="w-16 h-16 rounded-full bg-brand-cream mx-auto flex items-center justify-center text-brand-primary/50">
                    <ShoppingBag className="w-8 h-8" />
                  </div>
                  <h3 className="text-base font-bold text-brand-charcoal">Your cart is empty</h3>
                  <p className="text-xs text-brand-charcoal/40 max-w-sm mx-auto">
                    Add products from our product listing or recommendations below to get started!
                  </p>
                  <Link 
                    href="/products"
                    className="inline-flex items-center gap-2 px-6 py-2.5 bg-brand-primary hover:bg-brand-primary-dark text-white rounded-full text-xs font-black uppercase tracking-wider transition-all"
                  >
                ...
                <div className="divide-y divide-brand-charcoal/5">
                  {cartItems.map((item) => (
                    <div key={item.id} className="py-6 first:pt-0 last:pb-0 flex flex-col sm:flex-row items-center sm:items-start justify-between gap-5 transition-all">
                      
                      {/* Product details & thumbnail wrapper */}
                      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 flex-1">
                        {/* Thumbnail */}
                        <div className="w-24 h-24 rounded-2xl bg-brand-cream border border-brand-charcoal/5 overflow-hidden relative shrink-0 shadow-2xs">
                          <Image 
                            src={item.image} 
                            alt={item.name} 
                            fill 
                        ...
                        <div className="text-center sm:text-left space-y-3">
                          <div>
                            <h3 className="text-sm font-black text-brand-charcoal">{item.name}</h3>
                            <p className="text-[11px] text-brand-charcoal/40 font-bold mt-0.5">{item.desc}</p>
                          </div>
                          ...
                          <div className="flex items-center justify-center sm:justify-start gap-4">
                            <div className="flex items-center border border-brand-charcoal/10 rounded-full bg-white px-2 py-1 shadow-2xs">
                              ...
                              <span className="w-8 text-center text-xs font-black text-brand-charcoal select-none">
                                {item.quantity}
                              </span>
                            ...
                            <button 
                              ...
                              className="flex items-center gap-1 text-red-500 hover:text-red-700 text-[11px] font-black uppercase tracking-wider focus:outline-none"
                            >
                      ...
                      {/* Pricing block */}
                      <div className="text-right font-black text-brand-primary text-base min-w-[80px] sm:self-center">
          ...
          <div className="lg:col-span-4">
            <div className="bg-white border border-brand-charcoal/5 rounded-[24px] p-6 sm:p-8 shadow-xs space-y-6">
              ...
              {/* Breakdown */}
              <div className="space-y-4 text-xs font-bold text-brand-charcoal/70">
              ...
              <div className="space-y-3 pt-2">
                <Link 
                  href="/checkout"
                  className={cn(
                    "w-full block py-3.5 bg-brand-primary hover:bg-brand-primary-dark text-white font-black text-xs uppercase tracking-wider rounded-xl transition-all duration-300 shadow-md shadow-brand-primary/10 hover:-translate-y-0.5 text-center focus:outline-none",
                ...
                <Link 
                  href="/products"
                  className="w-full block py-3.5 rounded-xl border border-brand-primary hover:bg-brand-primary/5 text-brand-primary font-black text-xs uppercase tracking-wider text-center transition-all focus:outline-none"
                >
                  Continue Shopping
                </Link>
              </div>
        ...
        <div className="border-t border-brand-charcoal/5 pt-12 mb-8">
          <h2 className="text-lg font-black text-brand-charcoal tracking-wide mb-6">
            ...
            {recommendedItems.map((item) => (
              ...
              >
                ...
                <div className="flex items-center justify-between border-t border-brand-charcoal/5 pt-2.5">
                  <span className="text-xs font-extrabold text-brand-charcoal">₹{item.price}</span>
